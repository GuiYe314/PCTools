...
- 程序库与组合式工作模式：把 AI、Unity、网页和项目目录按顺序一键启动，可设置步骤等待、重复运行跳过和失败停止
- 文件夹、任务和命令中心的可保存快捷筛选
- GitHub 近 7/30/90 天热门项目、语言筛选、趋势快照和中文翻译
- 本机、Windows、CPU、内存使用、显卡及驱动、显示器、主板、BIOS、磁盘、运行时长、网卡、公网 IP、所在地和运营商信息检测
- DHCP/静态 IPv4 设置（应用时需要 Windows 管理员授权）
- 软件名称、功能入口、可自定义全局显示快捷键、开机启动和每日数据备份设置
...
当前数据结构版本为 `10`。持久化使用临时文件加替换的方式完成原子写入。
